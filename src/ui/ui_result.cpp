/**
 * ui_result.cpp — Detailed IC Test Result Screen (Deep Forest Emerald & Mint
 * Theme)
 *
 * Layout (320x240 landscape):
 * ┌──────────────────────────────────────────┐
 * │ [ ✓ PASSED ]                 7408 | 32ms │  Header banner (White text /
 * Mint pill) ├──────────────────────────────────────────┤ │
 * ┌──────────────────────────────────────┐ │ │ │  1 · [   74HC08   ] · 14 │ │
 * │ │  2 · [ G1:✓  G2:✓ ] · 13             │ │  Mint Visualization Card (h=142)
 * │ │  3 · [ G3:✓  G4:✓ ] · 12             │ │
 * │ │     [ ✓ ALL 4 GATES PASSED ]         │ │
 * │ └──────────────────────────────────────┘ │
 * ├──────────────────────────────────────────┤
...
static void draw_ic_visualization(lv_obj_t *parent, const ICTestResult *r) {
  ...
  // Physical IC Body (Compact height: 86px, shifted up to leave plenty of room
  // for badge)
  ...
  lv_obj_set_style_border_color(body, r->overall_pass ? CLR_NEON : CLR_ERROR,
                                0);
  ...
  for (uint8_t g = 0; g < ic->num_gates; g++) {
    offset +=
        snprintf(body_txt + offset, sizeof(body_txt) - offset, "G%d:%s ", g + 1,
                 r->gate_results[g].gate_pass ? LV_SYMBOL_OK : LV_SYMBOL_CLOSE);
  ...
  // Gate Summary Badge below IC (Placed clearly below body at bottom of card)
  ...
  lv_obj_set_style_bg_color(badge,
                            (bad_gates > 0) ? CLR_ERROR_DIM : CLR_BLUE_GLOW, 0);
  lv_obj_set_style_bg_opa(badge, LV_OPA_COVER, 0);
  lv_obj_set_style_border_color(badge,
                                (bad_gates > 0) ? CLR_ERROR : CLR_SUCCESS, 0);
  ...
  if (bad_gates > 0) {
    lv_label_set_text_fmt(lbl_badge,
                          LV_SYMBOL_WARNING " GOOD: %d  |  FAULTY: %d",
                          good_gates, bad_gates);
  } else {
    lv_label_set_text_fmt(lbl_badge, LV_SYMBOL_OK " ALL %d GATES PASSED",
                          good_gates);
  }
  lv_obj_set_style_text_font(lbl_badge, FONT_TINY, 0);
  lv_obj_set_style_text_color(lbl_badge, (bad_gates > 0) ? CLR_ERROR : CLR_TEXT,
                              0);
...
void ui_result_show(const ICTestResult *result) {
  ...
  lv_label_set_text(lbl_verdict, pass ? LV_SYMBOL_OK " TEST PASSED"
                                      : LV_SYMBOL_WARNING " TEST FAILED");
  ...
  char hdr_right[28];
  snprintf(hdr_right, sizeof(hdr_right), "%s  |  %lums", result->ic->ic_number,
  ...
  // ── Bottom Action Buttons (3 Buttons: HOME | RETEST | SAVE) ─────────────
  // 1. HOME button (Left)
  ...
  // 2. RETEST button (Center) - Highlighted in Emerald
  ...
  lv_timer_create(
      [](lv_timer_t *t) {
        if (g_result && g_result->overall_pass)
