/**
 * ui_history.cpp — Test History Screen
...
void ui_history_show(void) {
    ...
    lv_obj_set_size(btn_clr, 50, 22);
    lv_obj_align(btn_clr, LV_ALIGN_RIGHT_MID, 0, 0);
    ...
    if (cnt == 0) {
        ...
        for (int8_t i = (int8_t)cnt - 1; i >= 0; i--) {
            ...
            lv_obj_set_style_bg_color(row, ok ? CLR_SUCCESS_DIM : CLR_ERROR_DIM, 0);
            lv_obj_set_style_bg_opa(row, LV_OPA_30, 0);
            lv_obj_set_style_border_color(row, ok ? CLR_SUCCESS : CLR_ERROR, 0);
            ...
            lv_obj_set_style_text_color(lbl_num, CLR_TEXT_LABEL, 0);
            ...
            lv_obj_set_style_text_color(lbl_ic, CLR_NEON, 0);
            lv_obj_align(lbl_ic, LV_ALIGN_LEFT_MID, 30, 0);
            ...
            lv_obj_set_style_text_color(lbl_result, ok ? CLR_SUCCESS : CLR_ERROR, 0);
            lv_obj_align(lbl_result, LV_ALIGN_CENTER, 0, 0);
            ...
            lv_obj_set_style_text_color(lbl_ms, CLR_TEXT_LABEL, 0);
            lv_obj_align(lbl_ms, LV_ALIGN_RIGHT_MID, -4, 0);
        }
    }
    prefs.end();
